...
export function Dashboard() {
  ...
  const totalCreditsUsed = apiKeys.reduce(
    (sum: any, k: any) => sum + (k.credisConsumed ?? 0),
    0
  );
        ...
        {apiKeys.length > 0 && (
          <div>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-sm font-semibold text-accent-foreground">Your API Keys</h2>
              <Button variant="ghost" size="sm" asChild>
                <Link to="/apikeys" className="text-xs text-accent-foreground">
                  View all
                ...
                <tbody>
                  {apiKeys.slice(0, 5).map((key: any) => (
                    <tr key={key.id} className="border-b border-border/30 last:border-0">
                      <td className="px-4 py-3 font-medium text-muted-foreground">{key.name}</td>
                      <td className="px-4 py-3 font-mono text-xs text-muted-foreground">
                        {key.apiKey.slice(0, 12)}...{key.apiKey.slice(-4)}
                      </td>
                      <td className="px-4 py-3">
                        <span
                          className={`inline-flex items-center gap-1.5 text-xs font-medium ${key.disabled
                            ? "text-muted-foreground"
                            : "text-emerald-400"
                            }`}
                        >
                          <span
                            className={`size-1.5 rounded-full ${key.disabled ? "bg-muted-foreground" : "bg-emerald-400"
                              }`}
                          />
